(function() {
    ...
    if (window.audioAnalyzerOverlay) {
        alert('Audio Analyzer is already running on this page!');
        return;
    }
    ...
    overlay.innerHTML = `
        ...
        <div class="overlay-content">
            ...
            <div class="overlay-status">
                <span id="overlay-status-text">Ready to capture audio from this page</span>
            </div>
    ...
    overlay.querySelector('.close-btn').addEventListener('click', () => {
        if (analyzer && analyzer.audioContext) {
            analyzer.stopCapture();
        }
        overlay.remove();
        delete window.audioAnalyzerOverlay;
    });
    ...
    class OverlayAudioAnalyzer {
        ...
        findMediaElements() {
            ...
            // Filter for elements that are currently playing or have audio
            return mediaElements.filter(el => {
                return !el.paused || el.readyState >= 2; // Has audio data
            });
        }
        ...
        async startCapture() {
            try {
                ...
                if (mediaElements.length === 0) {
                    this.updateStatus('No audio/video found. Play audio first!', 'error');
                    setTimeout(() => {
                        this.updateStatus('Ready to capture audio from this page', 'info');
                    }, 3000);
                    return;
                }
                ...
                if (!this.mediaElement._audioSource) {
                    ...
                    // Connect to both analyser and destination (speakers)
                ...
                this.updateStatus('● Capturing audio from this page', 'success');

                // Update source info
                const sourceType = this.mediaElement.tagName.toLowerCase();
                const sourceURL = this.mediaElement.currentSrc || this.mediaElement.src;
                const sourceName = sourceURL.split('/').pop().split('?')[0] || sourceType;
                this.sourceText.textContent = sourceType === 'video' ? 'Video' : 'Audio';
                this.sourceText.title = sourceName;

                // Start visualization
                this.visualize();

                // Monitor if media stops
                const checkInterval = setInterval(() => {
                    if (this.mediaElement.ended || this.mediaElement.paused) {
                        clearInterval(checkInterval);
                        // Don't auto-stop, just let it continue analyzing
                    }
                }, 1000);
                ...
                if (error.name === 'InvalidStateError') {
                    this.updateStatus('Audio already connected. Refresh page to restart.', 'error');
                } else {
        ...
        stopCapture() {
            // Disconnect analyser but keep the audio playing
            if (this.source) {
                try {
            ...
            this.resetUI();
            this.updateStatus('Stopped. Ready to capture again.', 'info');
            this.clearCanvases();
        }
        ...
        updateVolume(dataArray) {
            ...
            if (normalizedVolume > 80) {
                this.volumeBar.style.backgroundColor = '#ff006e';
            } else if (normalizedVolume > 50) {
                this.volumeBar.style.backgroundColor = '#ffbe0b';
            } else {
                this.volumeBar.style.backgroundColor = '#00ff88';
            }
